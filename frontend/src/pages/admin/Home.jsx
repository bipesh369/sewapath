...
function Home() {
  ...
  useEffect(() => {
    getServices(1, 1)
      .then((res) => setTotalServices(res.pagination?.total ?? null))
      .catch(() => setTotalServices(null));
  }, []);

  const submitSearch = (e) => {
    e.preventDefault();
    if (searchText.trim()) {
      navigate(`/services?q=${encodeURIComponent(searchText.trim())}`);
    } else {
      navigate("/services");
    }
  };
  ...
  const submitGoal = (e) => {
    e.preventDefault();
    if (goalText.trim()) {
      navigate(`/services?goal=${encodeURIComponent(goalText.trim())}`);
    } else {
      navigate("/services");
    }
  };
  ...
  return (
    <>
      <div className="mx-auto max-w-1180px px-6 pt-16 pb-10 text-center md:px-12 md:pt-72px">
        <div className="mb-4 font-mono text-[12.5px] font-semibold tracking-[0.12em] text-crimson uppercase">
        ...
        <p className="mx-auto mb-9 max-w-540px text-[17px] leading-[1.6] text-ink/60">
          SewaPath turns any government office visit — passport, citizenship,
          land, license — into a clear, personal, step-by-step journey.
        </p>
          ...
          {tab === "search" ? (
            <form onSubmit={submitSearch} className="flex gap-2 p-2">
              <input
                ...
                className="flex-1 border-none bg-transparent px-4 py-3.5 text-[15px] outline-none"
              />
              <button
                type="submit"
        ...
        <div className="mb-12 flex flex-wrap justify-center gap-2.5 md:mb-52px">
          {CATEGORIES.map((c) => (
            <Chip key={c.title} onClick={() => navigate(`/services?q=${encodeURIComponent(c.query)}`)}>
              {c.icon} {c.title}
            </Chip>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-1000px px-6 pb-16 md:px-12 md:pb-90px">
        <div className="mb-7 text-center font-mono text-xs tracking-0.1em text-ink/60 uppercase">
          How SewaPath works
        </div>
        <div className="flex flex-wrap items-center justify-between gap-1">
          {FLOW.map((step, i) => (
            <div key={step.label} className="contents">
              ...
              {i < FLOW.length - 1 && (
                <div className="rail-seg mb-26px h-0.5 w-7 shrink-0" />
              )}
            </div>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-1080px px-6 pb-16 md:px-12 md:pb-90px">
        <h2 className="mb-2 text-[27px]">Browse by category</h2>
        <p className="mb-8 text-[15px] text-ink/60">
          Every service is verified against current office rules and updated
          monthly.
        </p>
        ...
        <div className="grid grid-cols-1 gap-18px sm:grid-cols-2 lg:grid-cols-4">
          {CATEGORIES.map((c) => (
            <button
              key={c.title}
              type="button"
              onClick={() => navigate(`/services?q=${encodeURIComponent(c.query)}`)}
            ...
            >
              ...
              <h3 className="mb-1.5 text-[15.5px]">{c.title}</h3>
              <p className="text-[13.5px] leading-1.5 text-ink/60">{c.desc}</p>
            </button>
          ))}
        </div>
      </div>
